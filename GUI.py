...
class GUI(QWidget):
    ...
    def initWindow(self):
        ...
        self.label = [QLabel(self) for _ in range(5)]
        for i in range(5):
            self.label[i].setScaledContents(True)  # 调整图片大小以适应QLabel，可以缩小图片
            self.label[i].setMaximumWidth(75)  # 设置最大宽度，可以控制图片的大小，避免过大覆盖整个窗口
            self.label[i].setMinimumHeight(75)  # 设置最小高度，可以控制图片的大小，避免过大覆盖整个窗口
            self.label[i].adjustSize()  # 自动调整大小以适应内容，包括图片和边框等所有内容

        hbox2.addWidget(self.label[0])
        hbox2.addWidget(self.label[1])
        hbox2.addWidget(self.label[2])
        hbox2.addWidget(self.label[3])
        hbox2.addWidget(self.label[4])
    ...
    def update_gui(self, read_server_str, write_server):
        ...
        if read_server_fn['type'] == 'start_mesg':
            ...
            self.welcome_label.setText(
                "欢迎使用LiquorDiceGame！\n,当前游戏轮次为第" + str(self.current_round) + "轮,您是" + str(
                    self.player_id) + "号玩家" "\n您的骰子结果为" + ','.join(
                    map(str, self.dice)) + "游戏总人数为:" + str(self.total_num) + "\n当前未到您的轮次，请您耐心等待!")

            self.pixmap = QPixmap('images/' + str(self.dice[0]) + '.jpg')
            self.label[0].setPixmap(self.pixmap)
            self.pixmap = QPixmap('images/' + str(self.dice[1]) + '.jpg')
            self.label[1].setPixmap(self.pixmap)
            self.pixmap = QPixmap('images/' + str(self.dice[2]) + '.jpg')
            self.label[2].setPixmap(self.pixmap)
            self.pixmap = QPixmap('images/' + str(self.dice[3]) + '.jpg')
            self.label[3].setPixmap(self.pixmap)
            self.pixmap = QPixmap('images/' + str(self.dice[4]) + '.jpg')
            self.label[4].setPixmap(self.pixmap)
            ...
            if self.finish_decide is not True:
                ...
                if self.last_face == -1:
                    self.welcome_label.setText(
                        "您是本轮第一个玩家" + " 您的骰子结果为" + str(self.dice) + "游戏总人数为:" + str(
                            self.total_num) + "\n当前轮到您的轮次，请从下面两个按钮中选择您要进行的操作")
                    self.openButton.setEnabled(False)
                else:
                    self.welcome_label.setText(
                        "上一个玩家的猜测为：" + str(self.last_num) + ' ' + str(self.last_face) + ' ' + str(
                            self.zhai) + "  您的骰子结果为" + str(
                            self.dice) + "\n当前轮到您的轮次，请从下面两个按钮中选择您要进行的操作")
            ...
            self.welcome_label.setText(
                "现在" + str(self.player_id) + "号玩家" + str(self.player_name) + "的猜测结果是" + str(
                    self.num) + ' ' + str(self.face) + ' ' + str(self.zhai) + ",游戏总人数为:" + str(
                    self.total_num) + "  您的骰子结果为" + str(self.dice) + "\n请您参考并等待进行下一步操作")
